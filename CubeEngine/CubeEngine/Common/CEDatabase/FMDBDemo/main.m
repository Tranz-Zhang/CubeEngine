...
int main (int argc, const char * argv[]) {
    ...
    @autoreleasepool {
        ...
        // get table info
        FMResultSet *resultSet = [db executeQuery:@"PRAGMA TABLE_INFO(test)"];
        while ([resultSet next]) {
            NSMutableString *str = [NSMutableString string];
            for (int i = 0; i < resultSet.columnCount; i++) {
                [str appendFormat:@"%s: %@ \t", sqlite3_column_name([resultSet.statement statement], i), [resultSet objectForColumnIndex:i]];
            }
            NSLog(@"%@", str);
        }
